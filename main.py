...
def _create_llm_client() -> Optional[LLMClient]:
    ...
    if not api_key:
        print("[提示] 未设置 DASHSCOPE_API_KEY 环境变量，使用关键词匹配模式")
        print("  设置方式: set DASHSCOPE_API_KEY=your-key  (Windows)")
        print("  获取 Key: https://dashscope.console.aliyun.com/apiKey")
        return None

    try:
        client = create_llm_client(api_key=api_key)
        print(f"[LLM] 已连接阿里 DashScope, 模型: {client.model}")
        return client
    except Exception as e:
        print(f"[警告] LLM 初始化失败: {e}，使用关键词匹配模式")
        return None
