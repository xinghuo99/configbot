...
class ChatSession:
    ...
    def _print_welcome(self):
        ...
        print("=" * 55)
        print(f"  {summary['name']} - 交互式聊天模式")
        print("=" * 55)
        print(f"  已加载: {summary['tools']['count']} 个工具, "
              f"{summary['skills']['count']} 个技能")
...
""")
    ...
    def _print_status(self):
        ...
        print(f"  历史条数: {len(self.history)}")
        print(f"  上下文键: {list(self.context.keys())}")
        print(f"  工具: {summary['tools']['count']} 个")
        print(f"  技能: {summary['skills']['count']} 个")
        print(f"  LLM: {'已配置' if summary['llm_configured'] else '未配置'}")
